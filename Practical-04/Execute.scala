class Parent
{
	...
	def Display_Parent()
	{
		...
		pub = 20
		println("Public variable is accessible Here (Parent) : " + prot)
		
		prot = 30
		println("Protected variable is accessible Here (Parent) : " + pub)
...
class Subclass extends Parent
{
	def Display_Subclass()
	{
		...
		println("Private variable is not accessible Here (Subclass) " + pvt)
